...
const AudioUploader = ({ handleUploadSuccess }) => {
  ...
  return (
    <div
      ...
      {
        loading &&
        <div style={{ width: +percentage + '%' }} className={`p-1 my-5 text-white font-semibold bg-primary transition-all duration-500 rounded-full text-center`}>
          {+percentage} %
        </div>
      }
      {
        !loading && isUploaded &&
        <div style={{ width: +percentage + '%' }} className={`p-1 my-5 text-white font-semibold bg-green-400 transition-all duration-500 rounded-full text-center`}>
          {+percentage} %
        </div>
      }
